...
import { BanknoteArrowUpIcon, FileClock, Hash, ShoppingCart, Info } from "lucide-react";
import { Link } from "react-router-dom";
import PropTypes from "prop-types";

ListCardSubastaActiva.propTypes = {
  data: PropTypes.array,
  isActive: PropTypes.bool.isRequired,
};

export function ListCardSubastaActiva({ data, isActive }) {
        ...
        <Card key={item.idObjeto} className="flex flex-col overflow-hidden">
          ...
          <CardContent className="flex-1 space-y-2 pt-4">
            ...
            <p className="flex items-center gap-2 text-sm text-muted-foreground">
              <BanknoteArrowUpIcon className="h-4 w-4 text-primary" />
              <b>Incremento Min: </b>
              {item.Incremento} 
            </p>
            <p className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button size="icon" className="size-8"
                    >
                      <Link to={`/subasta/interface/${item.idSubasta}`} 
                    state={{ objetoNombre: item.objeto?.Nombre, }}>
                      <ShoppingCart />
                    </Link>
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>Pujar</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            )}
          </div>
